...
import des_img from './assets/images/hero-desktop.jpg'
import mob_img from './assets/images/hero-mobile.jpg'
function App() {
  let inputRef = useRef();
  ...
  function clickHandler(){
    ...
    if((atIndex <= 0) || (dotIndex - atIndex) == 1 || email == '' ){
      setErr(true)
    }
    else{
      setErr(false)
  ...
  return (
    <>
     <div>
      <div className='flex md:flex-row flex-col back justify-center' >
          <div>
            <div className='svg h-full max-w-[830px] w-full md:px-32 pt-12 '>
              <div className='scale-[0.8] md:scale-100 md:px-10 px-0'>
              ...
              {win < 768 && <div className='mt-8'>
                <img src={mob_img} alt="" className='object-cover h-full w-full'/>
              </div>}
              <div className='mt-20 font-Josefin flex md:text-start flex-col  text-center mx-10'>
                <p className='font-light md:text-6xl tracking-widest text-Desaturated-Red text-5xl'>WE'RE</p>
                <p className='md:text-6xl text-4xl tracking-[0.3em] text-Dark-Grayish-Red font-semibold'>COMMING SOON</p>
                <p className='mt-8 text-Desaturated-Red tracking-wider md:text-base text-sm'>Hello fellow shoppers! We're currently building our new fashion store. 
                Add your email below to stay up-to-date with announcements and our launch deals.</p>
              </div>
              <form action='#' className='mt-10 flex mx-12 md:mx-0 relative items-center'>
                <input ref={inputRef} type="email" placeholder='Email Address'  className={` bg-transparent border rounded-full py-4 px-5 font-Josefin text-sm md:text-base w-full outline-none ${err ? 'border-Soft-Red border-2': 'border-Desaturated-Red'} `}/>
                {err && <div className='absolute md:right-28 top-4 right-20'>
                ...
                <button onClick={clickHandler} className='absolute btn md:px-11 px-8 py-5 rounded-full -right-1 top-0 shadow-xl '>
              ...
              {err && <p className='font-Josefin mt-3 ml-4 text-sm text-Soft-Red font-semibold text-center md:text-start'>Please provide a valid email</p>}
            </div>
          </div>
          {win >= 768 &&  <div>
              <img src={des_img} alt="" className='object-cover h-full'/>
          </div>}
      </div>
     </div>
    </>
